...
public:
                             ...
                             tf_listener_(tf_buffer_)
    {
        ...
        this->declare_parameter("urgent_goal_interval", 0.5);   // 增加到0.5秒，防止过载
...
private:
    ...
    void send_progressive_approach_goal(const geometry_msgs::msg::Point &final_target)
    {
        ...
        if (direction_magnitude > 0.1)
        {
            ...
            // 设置中间目标，距离不超过3米
            double step_distance = std::min(3.0, distance_to_target * 0.5);
            ...
            RCLCPP_INFO(this->get_logger(),
                        "🎯 渐进式靠近: 中间目标(%.2f, %.2f) | 步长: %.2fm | 最终距离: %.2fm",
                        intermediate_target.x, intermediate_target.y, step_distance, distance_to_target);
            
            send_follow_goal(intermediate_target, false, "渐进式靠近");
        }
    }
                          ...
                          const std::string &reason = "")
    {
        ...
        if (consecutive_failures_ >= max_retries_)
        {
            ...
            if (distance_to_drone > urgent_follow_distance_ * 1.5)
            {
