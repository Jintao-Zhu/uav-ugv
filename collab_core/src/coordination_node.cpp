#include <rclcpp/rclcpp.hpp> // 单纯的跟随节点，无避障功能
...
public:
    YahboomcarCoordinationNode() : Node("yahboomcar_coordination_node")
    {
        ...
        this->declare_parameter("kp_linear", 1.2);                // 线速度比例增益 (略降低以匹配车辆特性)
        this->declare_parameter("kp_angular", 2.8);               // 角速度比例增益 (略降低)
        this->declare_parameter("max_linear_vel", 2.5);           // 最大线速度 (匹配SDF中的限制)
        this->declare_parameter("max_angular_vel", 2.0);          // 最大角速度 (略保守)
        ...
        this->declare_parameter("angular_tolerance", 0.15);       // 角度容差 (更精确)
        this->declare_parameter("distance_tolerance", 0.3);       // 距离容差
        this->declare_parameter("emergency_brake_distance", 1.2); // 紧急制动距离
        this->declare_parameter("critical_safety_distance", 0.8); // 临界安全距离

        // Yahboomcar特定参数
        this->declare_parameter("wheel_separation", 0.169);         // 轮距 (从SDF获取)
        this->declare_parameter("wheel_diameter", 0.082);           // 轮径 (从SDF获取)
        ...
        // 发布yahboomcar速度指令 (使用命名空间)
        std::string cmd_vel_topic = "/" + vehicle_ns + "/cmd_vel";
        ugv_velocity_publisher_ = this->create_publisher<geometry_msgs::msg::Twist>(cmd_vel_topic, 10);
        ...
        RCLCPP_INFO(this->get_logger(), "  速度指令发布: %s", cmd_vel_topic.c_str());
        ...
        RCLCPP_INFO(this->get_logger(), "  emergency_brake_distance: %.2f m", this->get_parameter("emergency_brake_distance").as_double());
        RCLCPP_INFO(this->get_logger(), "  critical_safety_distance: %.2f m", this->get_parameter("critical_safety_distance").as_double());
...
private:
    ...
    rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr ugv_velocity_publisher_;
    ...
    void vehicle_following_control()
    {
        ...
        double emergency_brake_distance = this->get_parameter("emergency_brake_distance").as_double();
        double critical_safety_distance = this->get_parameter("critical_safety_distance").as_double();
        double distance_tolerance = this->get_parameter("distance_tolerance").as_double();
        double land_threshold = this->get_parameter("land_threshold").as_double();

        // 多级安全检查
        if (current_distance < critical_safety_distance)
        {
            critical_safety_stop("🚨 Yahboomcar临界安全距离！立即停止");
            return;
        }

        if (current_distance < emergency_brake_distance)
        {
            emergency_stop("⚠️ Yahboomcar紧急制动！距离过近");
            return;
        }

        // 无人机降落时的安全距离控制 - 针对四驱车特性调整
        ...
        {
            speed_factor = 1.3; // 四驱车可以更快追赶
        }

        // 降落模式下的速度调整 - 四驱车制动能力强
        if (drone_is_landing)
        {
            speed_factor *= 0.6; // 降落时适度降速，不如普通车那么保守
        }
        ...
        // 发布控制指令到Yahboomcar
        ugv_velocity_publisher_->publish(twist);
        ...
        RCLCPP_INFO(this->get_logger(),
                    "%s %s: 当前距离=%.2fm | 目标距离=%.2fm | %s | %s | yaw误差=%.1f°",
                    vehicle_status.c_str(), mode.c_str(), current_distance, target_distance,
                    direction.c_str(), safety_status.c_str(), yaw_error * 180.0 / M_PI);
    ...
    {
        // 在目标距离附近时的精细控制 - 四驱车精度更高
        double distance_error = current_distance - target_distance;

        // 微调控制，保持位置
        geometry_msgs::msg::Twist twist;
        twist.linear.x = 0.25 * distance_error; // 四驱车可以更精确调整
        twist.linear.y = 0.0;
    ...
    // 临界安全停止
    void critical_safety_stop(const std::string &reason)
    {
        geometry_msgs::msg::Twist stop_twist;
        stop_twist.linear.x = 0.0;
        stop_twist.linear.y = 0.0;
        stop_twist.angular.z = 0.0;
        ugv_velocity_publisher_->publish(stop_twist);

        double current_distance = sqrt(pow(latest_drone_pos_.x - latest_ugv_odom_.pose.pose.position.x, 2) +
                                       pow(latest_drone_pos_.y - latest_ugv_odom_.pose.pose.position.y, 2));

        RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 500,
                              "%s (距离: %.2fm < 临界安全距离: %.2fm)",
                              reason.c_str(), current_distance,
                              this->get_parameter("critical_safety_distance").as_double());
    }
    ...
    void emergency_stop(const std::string &reason)
    {
        ...
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                             "%s (距离: %.2fm < 制动距离: %.2fm)",
                             reason.c_str(), current_distance,
                             this->get_parameter("emergency_brake_distance").as_double());
    }
