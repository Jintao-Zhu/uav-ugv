...
class PatrolNode(BasicNavigator):
    ...
    def signal_handler(self, sig, frame):
        """处理Ctrl+C，就地停止后退出"""
        if self.stopping:
            self.get_logger().info("已在停止过程中，再次Ctrl+C强制退出")
            rclpy.shutdown()
            sys.exit(0)

        self.get_logger().info("收到终止信号，正在就地停止...")
        self.stopping = True

        # 取消当前导航任务，实现就地停止
        self.cancelTask()
        self.get_logger().info("已停止移动，程序退出")
    ...
    def nav_to_pose(self, target_pose):
        ...
        while not self.isTaskComplete() and not self.stopping:
            pass
