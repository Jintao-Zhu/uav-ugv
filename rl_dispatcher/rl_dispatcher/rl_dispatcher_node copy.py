...
class RLDispatchingEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        global_obs = [
            ...
            len(self.executing_tasks) / 8.0,     # 5（新增：执行中任务数）
...
class RLDispatcherNode(Node):
    ...
    def _init_rl_model(self):
        ...
        if self.mode == "train":
            ...
            # 新增：奖励阈值停止回调（作为EvalCallback的子回调）
            ...
            self.eval_callback = EvalCallback(
                ...
                verbose=1,
                callback_after_eval=self.stop_callback  # 关键修复：子回调配置
            )
    ...
    def start_training(self, total_timesteps=1000000):
        ...
        self.model.learn(
            ...
            callback=[self.checkpoint_callback, self.eval_callback]  # 移除独立的stop_callback
        )
    ...
    def _service_done(self, future, tid, robot):
        # === 新增：无论成功失败，只要有结果了，就从“发送中”名单移除 ===
        if tid in self.dispatching_task_ids:
            self.dispatching_task_ids.remove(tid)
