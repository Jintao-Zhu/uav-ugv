...
# 1.17 调用infer模式，明明只接收了一个目标，但是发布了一大堆任务
# ===================== 引入双引擎 =====================
...
class RLDispatcherNode(Node):
    ...
    def infer_dispatch_callback(self):
        """真实环境下的周期性调度"""
        if not self.rl_env.pending_tasks: return
        ...
        task = self.rl_env.pending_tasks[0] # 这里只看第一个，不pop，等服务成功再处理
    ...
    def target_callback(self, msg):
        ...
        if tid in self.processed_ids: return
