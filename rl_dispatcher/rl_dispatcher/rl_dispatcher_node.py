#!/usr/bin/env python3
#zjt,1月21日 跑出了接近贪心的结果
# 适配需求：8任务必须完成+无待命点返回+超时不丢任务
# 核心优化：修复任务集中分配+奖励过大+标准差高问题
# 新增优化：链式距离计算 + 时间拥堵惩罚 + 54维状态空间
# pzh:增加全局任务队列模式，已经比rmf的原生算法快了，9min11s < 9min32s,此外纯贪心耗时14分45秒
# 结果在PPO17
import sys
import os
...
class RLDispatcherNode(Node):
    ...
    def _init_rl_model(self):
        ...
        if self.mode == "train":
            ...
            self.checkpoint_callback = CheckpointCallback(save_freq=10000, save_path="./rl_models/", name_prefix="ppo")
                ...
                for i, t in enumerate(self.rl_env.pending_tasks):
                    if t["task_id"] == tid:
                        ...
                        if r_state["current_target"]:
                            anchor_x, anchor_y = r_state["current_target"]["x"], r_state["current_target"]["y"]
                        else:
