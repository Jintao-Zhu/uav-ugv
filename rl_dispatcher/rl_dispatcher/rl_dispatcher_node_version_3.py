...
# 7. 奖励系数 (⚡️ 1/10 稀疏奖励版：验证信号强度对收敛的影响 ⚡️)
    ...
    def step(self, action):
        ...
        def clip_reward(r, min_r=-2.0, max_r=2.0): # 裁剪范围也减半
        ...
        if action == wait_action_idx:
            ...
            if robot["idle"]:
                ...
                if robot["current_target"]:
                    ax, ay = robot["current_target"]["x"], robot["current_target"]["y"]
                else:
                    ax, ay = self.robot_positions[selected_robot].x, self.robot_positions[selected_robot].y
                
                def sort_by_distance(t):
                    return np.sqrt((ax - t["x"])**2 + (ay - t["y"])**2)
...
class RLDispatcherNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("🚀 初始化 RL 调度节点（消融实验：温和奖励版）...")
    ...
    def _init_rl_model(self):
        # ⚠️ 注意：修改保存路径，防止覆盖全量RL模型
        model_path = "./rl_models/dispatching_ppo_mild"
        ...
        if self.mode == "train":
            ...
            self.get_logger().info("📚 初始化 PPO 模型 (温和奖励版)...")
            ...
            # ⚠️ 注意：CheckPoint保存路径也改为 mild
            self.checkpoint_callback = CheckpointCallback(save_freq=10000, save_path="./rl_models/mild/", name_prefix="ppo")
            ...
            self.eval_callback = EvalCallback(
                ...
                eval_freq=5000,
                n_eval_episodes=5,
                best_model_save_path="./rl_models/mild/best/",
                verbose=1,
    ...
    def start_training(self, total_timesteps=1000000):
        ...
        self.get_logger().info(f"🔥 开始温和奖励消融实验训练 {total_timesteps} 步...")
        start_t = time.time()
        ...
        # ⚠️ 保存到 mild 路径
        self.model.save("./rl_models/dispatching_ppo_mild")
    ...
    def infer_dispatch_callback(self):
        ...
        if action == wait_action_idx:
            ...
            if robot["task_queue"]:
                anchor_x = robot["task_queue"][-1]["x"]
                anchor_y = robot["task_queue"][-1]["y"]
            elif robot["current_target"]:
                anchor_x = robot["current_target"]["x"]
                anchor_y = robot["current_target"]["y"]
            else:
            ...
            if res.success:
                ...
                for i, t in enumerate(self.rl_env.pending_tasks):
                    if t["task_id"] == tid:
                        ...
                        # 队列重排序逻辑 (保留)
                        if r_state["current_target"]:
                            ax, ay = r_state["current_target"]["x"], r_state["current_target"]["y"]
                        else:
                            ax, ay = self.rl_env.robot_positions[robot].x, self.rl_env.robot_positions[robot].y
