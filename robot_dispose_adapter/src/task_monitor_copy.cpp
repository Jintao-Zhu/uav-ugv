...
std::string task_status_to_string(TaskStatus status) {
    switch(status) {
        case EXECUTING: return "EXECUTING(执行中)";
        case COMPLETING: return "COMPLETING(处置中)";
...
public:
    TaskMonitor() : Node("task_monitor") {
        ...
        // ========== 新增：小车状态发布器（推送空闲/忙碌信号） ==========
        robot_status_pub_ = this->create_publisher<std_msgs::msg::String>(
            "/robot/status", 10);
        RCLCPP_INFO(this->get_logger(), "✅ 已创建小车状态发布器：/robot/status");
...
private:
    ...
    void fleet_state_callback(const rmf_fleet_msgs::msg::FleetState::SharedPtr msg) {
        ...
        for (const auto& robot : msg->robots) {
            ...
            if (!rmf_task_id.empty()) {
                status_msg.data = robot_name + ",busy";
            } else {
                status_msg.data = robot_name + ",idle";
            }
            robot_status_pub_->publish(status_msg);
            
            RCLCPP_DEBUG(this->get_logger(), 
                        "🔍 小车[%s] 位置: (%.2f, %.2f) | RMF任务ID: %s | 状态: %s",
                        robot_name.c_str(), robot_position.x, robot_position.y, 
                        rmf_task_id.c_str(), 
                        rmf_task_id.empty() ? "idle" : "busy");
        }
    }
    ...
    void publish_completion(const std::string& task_id, const std::string& robot_name, const std::string& waypoint) {
        ...
        robot_status_pub_->publish(status_msg);
        RCLCPP_INFO(this->get_logger(),
                   "📤 小车[%s]已标记为空闲，推送状态到/robot/status",
                   robot_name.c_str());
