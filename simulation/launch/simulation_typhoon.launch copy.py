...
def kill_existing_gazebo(context, *args, **kwargs):
    ...
    os.system("pkill gazebo")
    os.system("pkill px4")  # 关闭可能存在的PX4进程
    os.system("pkill MicroXRCEAgent")  # 关闭残留的Agent进程
    return []
...
def generate_launch_description():
    ...
    env_vars = [
        ...
        # 设置PX4仿真模型为iris_depth_camera
        SetEnvironmentVariable(
            name='PX4_SIM_MODEL',
            value='iris_depth_camera'  # 修改模型名称
        )
    ]
    ...
    gazebo = ExecuteProcess(
        cmd=[
            'gazebo',
            '/home/suda/drone_ugv_ws/src/simulation/worlds/coordination_world.world',
    ...
    # 无人机模型路径 - 修改为iris_depth_camera的SDF文件路径
    # 注意：确认实际路径是否正确，可能需要调整
    ...
    spawn_drone = Node(
        ...
        arguments=[
            ...
            '-entity', 'iris_depth_camera',  # 修改实体名称
            '-x', '0', '-y', '0', '-z', '0.3'  # 适当提高高度，避免与地面碰撞
        ]
    )
